...
public class LogicHandle {
	...
	private boolean logicConnector(String str, List<Boolean> temp)
		...
		if (conn != null) {
			try {
				if (conn.equals(Keyword.getKeyword("and")))
					result = temp.get(0) && temp.get(1);
				if (conn.equals(Keyword.getKeyword("or")))
					result = temp.get(0) || temp.get(1);
			} catch (Exception e) {
	...
	private boolean ifCondition(String str) throws DefinedException {
		...
		for (String con : conList) {
			...
			if (determine.equals(Keyword.getKeyword("equals")))
				temp.add(b);
			if (determine.equals(Keyword.getKeyword("notEquals")))
				temp.add(!b);
		}
